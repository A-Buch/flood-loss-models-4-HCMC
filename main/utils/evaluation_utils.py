...
def reverse_probability_scores(df, predproba_colname="y_proba", pred_colname_value=("y_pred", 0.0)):
    ...
    df[predproba_colname] = df.apply(
        lambda x: np.abs(x[predproba_colname] - 1) if x[pred_colname_value[0]] == pred_colname_value[1] else x[predproba_colname],  # else x["y_proba"] -> dont do anything for predicted damage-cases
        axis=1).round(3)
