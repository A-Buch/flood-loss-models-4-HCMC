...
main_logger = "__feature_extraction_bred__"
...
Path("../../model_results/models_trained/degree_of_loss/nested_cv_models").mkdir(parents=True, exist_ok=True)
Path("../../model_results/models_trained/degree_of_loss/final_models").mkdir(parents=True, exist_ok=True)
Path("../../model_results/models_evaluation/degree_of_loss").mkdir(parents=True, exist_ok=True)
Path("../../model_results/selected_features/degree_of_loss").mkdir(parents=True, exist_ok=True)
...
for pipe_name in pipelines:
    ...
    ## run R model
        ...
        f.plot_r_learning_curve(df_Xy, target, f"../../model_results/models_evaluation/degree_of_loss/learning_curves{target}_{model_name}.png")
...
f.boxplot_outer_scores_ncv(
    models_scores, outfile=f"../../model_results/models_evaluation/degree_of_loss/boxplot_scores4ncv_{target}.png", target_name=target_plot
)


# store avergaed scores and std for later usage
## TODO remove overhead  -> store avergaed scores and std during loop
xgb_model_evaluation = pd.DataFrame(models_scores["XGBRegressor"]).mean(
    axis=0
...
model_evaluation = pd.concat(
    [en_model_evaluation, en_model_evaluation_std, crf__model_evaluation, crf_model_evaluation_std, xgb_model_evaluation, xgb_model_evaluation_std],
    axis=1,
)
model_evaluation.columns = [
    "ElasticNet_score",
    "ElasticNet_score_std",
    "cforest_score",
    "cforest_score_std",
    "XGBRegressor_score",
    "XGBRegressor_score_std",
]
...
f.plot_stacked_feature_importances(
    df_feature_importances_plot[
        [
            "ElasticNet_importances_weighted",
            "cforest_importances_weighted",
            "XGBRegressor_importances_weighted",
        ]
    ],
