...
## settings for cv
kfolds_and_repeats = 10, 5 # 5,5 # 10, 5 # 5, 1 # 3, 1  # <k-folds, repeats> for nested cv
...
for pipe_name in pipelines:
    ...
    # with contextlib.suppress(Exception):
    #     df_Xy.drop(["hh_monthly_income_euro"], axis=1, inplace=True)
    ...
    if model_name != "cforest":
        ...
        f.plot_learning_curves(
            ...
            f"../../model_results/models_evaluation/degree_of_loss/learning_curves_{target}_{model_name}.png", 
            model_name)
    ...
    ## run R model
        ...
        # models_trained_ncv = r_model_fit_ncv(y=df_Xy[target], X=df_Xy.drop(target, axis=1), r_algorithm_name="cforest",
        #             k_folds=kfolds_and_repeats[0], repeats=kfolds_and_repeats[1]  )
