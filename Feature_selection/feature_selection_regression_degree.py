...
targets = [("degree of rcloss", "degree of rcloss"), ("rbfred","rbred")]
...
OUTPATH_EVAL = Path(s.OUTPATH_EVAL, "degree_of_loss")
OUTPATH_FEATURES = Path(s.OUTPATH_FEATURES, "degree_of_loss")
OUTPATH_FINALMODEL = Path(s.OUTPATH_ESTIMATORS, "final_models", "degree_of_loss")
OUTPATH_ESTIMATOR = Path(s.OUTPATH_ESTIMATOR, "nested_cv_models", "degree_of_loss")
OUTPATH_FIGURES = Path(s.OUTPATH_FIGURES, "models_evaluation", "degree_of_loss")
...
## init variabels to store model outputs
eval_sets = {}
models_trained = {}
...
for pipe_name in pipelines:
    ...
    pipe = joblib.load(f"{s.OUTPATH_UTILS}/{pipe_name}.pkl")
    ...
    if model_name != "cforest":
        ...
        model_evaluation_results = me.negate_scores_from_sklearn_cross_valdiate(  # TODO impl this directly as method in ModelEvalaution()
        ...
        logger.info(f"Parameter sets of estimators on outer test-sets:") 
        ...
        ## TODO make selection of final model as method for ModelFitting() or ModelEvaluation()
        ...
        f.plot_learning_curves(
            ...
            f"{OUTPATH_EVAL}/learning_curves_{target}_{model_name}.png", 
            model_name)
        ...
        with contextlib.suppress(Exception):
            ...
            outfile = f"{OUTPATH_EVAL}/regression_coefficients_{model_name}_{target}.xlsx"
    ...
    ## run R model
       ...
       ## needed to get R estimators 
        ...
        ## Final CRF mode TODO make func as mf.r_final_model()
        robjects.r("""
...
f.boxplot_outer_scores_ncv(
    models_scores,
    outfile=f"{OUTPATH_EVAL}/boxplot_scores4ncv_{target}.png",
...
outfile = f"{OUTPATH_EVAL}/performance_{target}.xlsx"
...
f.plot_stacked_feature_importances(
    ...
    outfile=f"{OUTPATH_EVAL}/feature_importances_{target}.png"
)
...
final_feature_names = df_feature_importances_w["weighted_sum_importances"].sort_values(ascending=False).index##[:10]
 # predictor names +further features for futher analysis (eg content value need it to calc abl loss after BN)
final_feature_names.insert(-1,"shp_business_limitation").insert(-2,"shp_sector")
...
categorical = [] #  eg. ["flowvelocity", "further_variables .."]
ncols = 3
nrows = len(most_important_features[:10])# NOTE keep as 10 for rugs
idx = 0
...
for feature in most_important_features[:9]:
    for model_name, color, idx_col in zip(["ElasticNet", "cforest", "XGBRegressor"], ["steelblue", "darkblue", "grey"], [0, 0, 0]):
        ...
        p = f.plot_partial_dependence(
            ...
            categorical=[],
            outfile=f"{OUTPATH_EVAL}/pdp_{target}.png",
            **feature_info
            )
        p
        plt.ylim(0,30)
...
f.plot_residuals(
    ...
    outfile=f"{OUTPATH_EVAL}/residuals_{target}.png"
)
